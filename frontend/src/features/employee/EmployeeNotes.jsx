...
export default function EmployeeNotes() {
  ...
  const loadNotes = async () => {
    try {
      setLoading(true);
      const response = await employeeNoteService.getByEmployeeId(employeeId);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingNote) {
        await employeeNoteService.update(
          editingNote.note_id,
        ...
        await employeeNoteService.create(
  ...
  const handleToggleComplete = async (noteId) => {
    try {
      await employeeNoteService.toggleComplete(noteId);
      loadNotes();
    } catch (error) {
  ...
  const handleDelete = async (noteId) => {
    ...
    try {
      await employeeNoteService.delete(noteId);
  ...
  if (loading) {
    return (
      <Layout
        ...
        breadcrumb={["Home", "Employees", "Notes"]}
      >
  ...
  return (
    <Layout title="Employee Notes" breadcrumb={["Home", "Employees", "Notes"]}>
      <div className="customer-notes-container">
        {/* Header */}
        <div className="notes-header">
          <div className="notes-header-left">
            <h2>Notes for: {getEmployeeName()}</h2>
